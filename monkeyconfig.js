 ...
 * Copyright (c) 2021-2021 Whale Mo (kouga.us)
 */
...
MonkeyConfig.HTML = {
  ...
  'checkbox': (name, options, data) => `<input id="__MonkeyConfig_field_${name}" type="checkbox" name="${name}" />`,
  'custom': (name, options, data) => options.html,
  'number': (name, options, data) => `<input id="__MonkeyConfig_field_${name}" type="text" class="__MonkeyConfig_field_number" name="${name}" />`,
  'select': function (name, options, data) {
    ...
    if (!options.multiple) {
      ...
      if (!/^radio/.test(options.variant)) {
        /* Select element */
        html += `<select id="__MonkeyConfig_field_${name}" class="__MonkeyConfig_field_select" name="${name}">`;
        ...
        /* Radio buttons */
        for (var value in choices) {
          html += `<label><input type="radio" name="${name}" value="${MonkeyConfig.esc(value)}" />${choices[value]}</label>'`
          if (/ column/.test(options.variant))
            html += '<br />'
        }
      }
    } else {
  ...
  'text': function (name, options, data) {
    if (options.long && !isNaN(options.long))
      return `<textarea id="__MonkeyConfig_field_${name}" class="__MonkeyConfig_field_text" rows="${options.long}" 'name="${name}"></textarea>`;
    else
      return `<input id="__MonkeyConfig_field_${name}" type="text" class="__MonkeyConfig_field_text" name="${name}" />`;
  }
};
